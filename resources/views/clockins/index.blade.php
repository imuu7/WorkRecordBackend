...
@section('content')
    <div class="container-fluid container-fixed-lg">
        <h3 class="page-title">@lang('models/clockins.plural')</h3>
    ...
    <div id="shadow-form" style="display: none">
        {!! Form::open(['route' => 'clockins.store', 'method' => 'PATCH']) !!}
...
<script>
    function ajaxCU(fill) {
        ...
        if (fill) {
            ...
            form.attr('action', '{{ route("clockins.store") }}/' + fill['id'])
            method.val('PATCH')
        } else {
            form.attr('action', '{{ route("clockins.store") }}')
            method.val('POST')
            form.trigger("reset")
        }
        $('#shadow-form').show()
