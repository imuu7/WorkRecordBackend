...
@section('content')
    <div class="container-fluid container-fixed-lg">
        <h3 class="page-title">@lang('models/configTables.plural')</h3>
    ...
    <div id="shadow-form" style="display: none">
        {!! Form::open(['route' => 'configTables.store', 'method' => 'PATCH']) !!}
...
<script>
    function ajaxCU(fill) {
        ...
        if (fill) {
            ...
            form.attr('action', '{{ route("configTables.store") }}/' + fill['id'])
            method.val('PATCH')
        } else {
            form.attr('action', '{{ route("configTables.store") }}')
            method.val('POST')
        }
        $('#shadow-form').show()
    }
